...
state.responses = []
state.model = joblib.load('depression.pkl') 
...
if submit_button:
    state.prediction = state.model.predict(convertor(state.responses))
    st.markdown("<h1 style = 'text-align: center;'> Model Predictions </h1>", unsafe_allow_html=True)  
    st.pyplot(plot_graph(state.prediction[0], 'Depression'))
